...
class OrderPaginator:
	...
	async def show_page(self, callback: types.CallbackQuery, state: FSMContext, language='eng', courier_id=-1,
						edit_msg=False):
		...
		keyboard = await self.get_keyboard(courier_id=courier_id, page=current_page, language=language,
										   prefix=self.prefix)
